...
public class DiagramTransitionRunnable extends AbstractProcessingCommands<DDiagram> {
  ...
  public String getName() {
    return Messages.DiagramTransitionRunnable_10;
  }
  ...
  protected IStatus proceedDiagram(final DRepresentation diagram, final IProgressMonitor monitor) {
    ...
    monitor.beginTask(NLS.bind(Messages.DiagramTransitionRunnable_11, diagramName), 3);
    monitor.setTaskName(NLS.bind(Messages.DiagramTransitionRunnable_12, diagramName));

    final IContext context = getContext();

    if (!(diagram instanceof DSemanticDecorator)) {
      return new Status(IStatus.ERROR, Activator.PLUGIN_ID, Messages.DiagramTransitionRunnable_13);
    }
    ...
    if ((sourceSemantic == null) || sourceSemantic.eIsProxy()) {
      return new Status(IStatus.ERROR, Activator.PLUGIN_ID, Messages.DiagramTransitionRunnable_14);
    }

    // Retrieve the current session
    final Session session = DiagramHelper.getService().getSession(diagram);
    if (session == null) {
      return new Status(IStatus.ERROR, Activator.PLUGIN_ID, Messages.DiagramTransitionRunnable_15);
    }
    ...
    if ((description == null) || description.eIsProxy()) {
      return new Status(IStatus.ERROR, Activator.PLUGIN_ID, Messages.DiagramTransitionRunnable_16);
    }
    ...
    if (targetDiagram == null) {
      return new Status(IStatus.ERROR, Activator.PLUGIN_ID, Messages.DiagramTransitionRunnable_20);
    }
    return new Status(IStatus.OK, Activator.PLUGIN_ID, NLS.bind(Messages.DiagramTransitionRunnable_21, diagramName));
  }
  ...
  protected IStatus retrieveTargetDiagram(DRepresentation diagram, Session session, EObject sourceSemantic,
    ...
    if ((allocatingDescription == null) || allocatingDescription.eIsProxy()) {
      return (new Status(IStatus.ERROR, Activator.PLUGIN_ID,
          NLS.bind(Messages.DiagramTransitionRunnable_22, EObjectExt.getText(diagram))));
    }
    ...
    if (targetSemantic == null) {
      return (new Status(IStatus.ERROR, Activator.PLUGIN_ID,
          Messages.DiagramTransitionRunnable_23));
    }
    ...
    if (targetDiagram == null) {
      return (new Status(IStatus.ERROR, Activator.PLUGIN_ID, Messages.DiagramTransitionRunnable_24));
    }
  ...
  private IStatus rollbackDiagram(final DRepresentation diagram) {
    ...
    final Session session = DiagramHelper.getService().getSession(diagram);
    if (session == null) {
      return new Status(IStatus.ERROR, Activator.PLUGIN_ID, Messages.DiagramTransitionRunnable_25);
    }
