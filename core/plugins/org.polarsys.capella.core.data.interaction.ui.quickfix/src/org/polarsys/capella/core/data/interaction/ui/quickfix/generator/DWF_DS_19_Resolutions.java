...
public class DWF_DS_19_Resolutions extends AbstractMarkerResolutionGenerator {
  ...
  protected IMarkerResolution[] doGetResolutions(IMarker marker) {
    ...
    resolutions.add(new CapellaElementGoToResolver(Messages.DWF_DS_19_Resolutions_0, invokedOperation));

    NamedElement sequenceMessageSourceFunction = sequenceMessage.getSendingFunction();
    resolutions.add(new CapellaElementGoToResolver(Messages.DWF_DS_19_Resolutions_1, sequenceMessageSourceFunction));

    NamedElement sequenceMessageTargetFunction = sequenceMessage.getReceivingFunction();
    resolutions.add(new CapellaElementGoToResolver(Messages.DWF_DS_19_Resolutions_2, sequenceMessageTargetFunction));

    AbstractFunction sourceFunction = FunctionalExchangeExt.getSourceFunction((FunctionalExchange) invokedOperation);
    resolutions.add(new CapellaElementGoToResolver(Messages.DWF_DS_19_Resolutions_3, sourceFunction));

    AbstractFunction targetFunction = FunctionalExchangeExt.getTargetFunction((FunctionalExchange) invokedOperation);
    resolutions.add(new CapellaElementGoToResolver(Messages.DWF_DS_19_Resolutions_4, targetFunction));
    ...
    // Delete.
    resolutions.add(new DeleteCommandResolver(Messages.DWF_DS_19_Resolutions_5, SequenceMessageExt.getStartedExecution(sequenceMessage)));
