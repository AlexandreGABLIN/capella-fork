...
public class DWF_CA_07_Resolutions extends AbstractMarkerResolutionGenerator {
  ...
  protected IMarkerResolution[] doGetResolutions(IMarker marker) {
    ...
    if(capability != null) {
      resolutions.add(new CapellaElementGoToResolver(Messages.DWF_CA_07_Resolutions_0, capability));
    }
    
    
    Component element = (Component) modelElements.get(1);
    if(element != null) {
      resolutions.add(new CapellaElementGoToResolver(Messages.DWF_CA_07_Resolutions_1, element, NamingHelper.getTitleLabel(element)));
    }
