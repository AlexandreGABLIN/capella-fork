        ...
        for (SequenceMessage message : allocation.getInvokingSequenceMessages()) {
          if (message.getKind() != MessageKind.REPLY) {
            ...
            if (!expectedSendProtocols.isEmpty() && !expectedSendProtocols.contains(allocation.getSendProtocol())) {
              ...
              IStatus status =  ctx.createFailureStatus(eObj, mechanism, expected, Messages.ExchangeItemAllocationProtocol_CommunicationLinkProtocol_Sender, message, message.getKind(), hasReply ? Messages.ExchangeItemAllocationProtocolSequence_1 : Messages.ExchangeItemAllocationProtocolSequence_2, scenario);
              statuses.add(status);
            }
            ...
            if (!expectedReceiveProtocols.isEmpty() && !expectedReceiveProtocols.contains(allocation.getReceiveProtocol())) {
              ...
              IStatus status =  ctx.createFailureStatus(eObj, mechanism, expected, Messages.ExchangeItemAllocationProtocol_CommunicationLinkProtocol_Receiver, message, message.getKind(), hasReply ? Messages.ExchangeItemAllocationProtocolSequence_4 : Messages.ExchangeItemAllocationProtocolSequence_5, scenario);
              statuses.add(status);
            }
          }
        }
