              ...
              if (!descriptions.isEmpty()) {
                if (descriptions.size() > 1) {
                  ...
                  InputDialog dialog = new InputDialog(activeShell,
                      NLS.bind(Messages.OpenRelatedDiagramEditPolicy_1, MessageTranslator.INSTANCE.getMessage(description,
                              new IdentifiedElementQuery(description).getLabel())),
                      Messages.OpenRelatedDiagram_Message, newName, validator);
                  dialog.open();
                  if (dialog.getReturnCode() == Window.OK) {
